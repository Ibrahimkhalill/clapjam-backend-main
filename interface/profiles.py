...
class ProfileHandler:
    ...
    def update_birthdate(self, date: str | int) -> dict | None:
        try:
            if isinstance(date, str):
                print(f"Parsing birthdate from string: {date}")
                ...
                print(f"Parsing birthdate from timestamp: {date}")
    ...
    def update_primary(self, name: str, nickname: str, birthdate) -> dict | None:
        is_valid, errors = self.validate_primary(name, nickname)
        self.error_messages += errors
        if is_valid:
            self.update_name(name)  
            self.update_nickname(nickname)
            self.update_birthdate(int(birthdate) if isinstance(birthdate, str) else birthdate)
            return self.primary
