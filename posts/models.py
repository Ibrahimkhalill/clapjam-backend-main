...
#             viewer_liked = self.likes.filter(user=viewer).exists()
...
#         images = [u.file.url for u in self.imageurls.all()]
#         videos = [u.file.url for u in self.videourls.all()]
...
#     file = models.ImageField(upload_to='post_images/', blank=True, null=True)  # Stores image files

# class PostVideoUrl(models.Model):
#     image = models.ForeignKey(
#         PostMetaData, on_delete=models.CASCADE, related_name='videourls')
#     file = models.FileField(upload_to='post_videos/', blank=True, null=True)  # Stores video files
...
#         pics = [pic.file.url for pic in self.pics.all()] if hasattr(self, 'pics') else list()
#         return dict(
#             comment_id=self.id,
#             user=dict(
#                 user_id=self.user.id,
...
#         return dict(
#             reply_id=self.id,
#             comment_id=self.comment.id,  # Added comment_id
#             user=dict(
#                 user_id=self.user.id,
#                 name=self.user.get_full_name(),
#                 profile_pic_url=self.user.pics.profile_pic_url
...
#     file = models.ImageField(
#         upload_to='comment_reply_images/', blank=True, null=True,
#         validators=[FileExtensionValidator(allowed_extensions=['jpg', 'jpeg', 'png'])]
#     )
...
class PostMetaData(models.Model):
    ...
    @property
    def all_comments(self) -> dict:
        comment_list = [comment.details for comment in self.comments.all().order_by('-created_at')] if hasattr(self, 'comments') else list()
        return dict(
            total=self.total_comments,
            comment_list=comment_list
        )
...
class PostImageUrl(models.Model):
    image = models.ForeignKey(
        PostMetaData, on_delete=models.CASCADE, related_name='imageurls')
    file = models.ImageField(upload_to='post_images/', blank=True, null=True)

class PostVideoUrl(models.Model):
    image = models.ForeignKey(
        PostMetaData, on_delete=models.CASCADE, related_name='videourls')
    file = models.FileField(upload_to='post_videos/', blank=True, null=True)
